...
def render(
    ...
    Template syntax (Mustache):
      ...
      Use {{key}} to insert a config value. Keys are the config's YAML paths
      with the leading slash removed and forward slashes separating levels:

        Config path         Template tag
        /grid/x/min     ->  {{grid/x/min}}
        /material/k     ->  {{material/k}}

      Quantities are rendered as their full string, e.g. "-10.0 centimeter".
      To get a plain number in specific units, use a conversion expression in
      the config: $(${/grid/x/min}.to('mm').magnitude)

    \b
    Examples:

      # Single-instance render
      powerconf render CONFIG.yaml template.txt.template output.txt

      # Batch render (output becomes a directory; one file per instance)
      powerconf render CONFIG.yaml template.txt.template output.d
    """
